...
use domain::{
    build_calculation::{
        stored_item::{StoredItem, StoredItemError},
...
pub enum ImportPobError {
    ...
    #[error("domain item convert to stored item error: {0}")]
    Convert(#[from] StoredItemError),
}
...
#[cfg(test)]
mod tests {
    use domain::build_calculation::stored_item::ItemInfo;
