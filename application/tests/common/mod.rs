use application::{build_calculation::{import_pob::{import_build_from_pob, import_build_from_pob_first_itemset}, BuildInfo}, stash_receiver::StashReceiver, storage::postgresql::{builds::{BuildData, BuildRepository}, items::ItemRepository, MIGRATOR}};
...
pub async fn setup_db() -> anyhow::Result<TestContext> {
    let pool = PgPoolOptions::new()
        .max_connections(5)
        .connect("postgres://khooj@localhost/tests").await?;
    ...
    build_repo.upsert_build(BuildData {
        id: Uuid::new_v4(),
        processed: false,
        created_at: Utc::now(),
        updated_at: Utc::now(),
    ...
    Ok(TestContext { item_repo, build_repo })
}
